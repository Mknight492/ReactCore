...
class IdentityPage extends React.Component {
  ...
  handleSubmit(event) {
    ...
    const { user } = this.state;
    IdentityRegister(user).then(promise => {
      console.log("pass");
    });
  }
...
const connectedIdentityPage = connect(mapStateToProps)(IdentityPage);
export { connectedIdentityPage as IdentityPage };
...
async function IdentityRegister(user) {
  ...
  console.log(data);
  const loggedIn = await fetch("/Account/Register", {
    method: "POST",
    headers: {
